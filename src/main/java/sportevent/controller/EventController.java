...
public class EventController {
    ...
    public ResponseEntity<?> createEvent(@PathVariable("name") String name, @RequestParam("description") String description, @RequestParam("date") Date date, @RequestParam("promoterid") Long promoterId){
        Promoter eventPromoter = promoterRepository.findById(promoterId);
        Event savedEvent = eventRepository.save(new Event(date, name, description, eventPromoter));
        if (savedEvent!=null){
